...
struct AabbWorld {
	...
	float *step_start_y;
	/*
	 * Contact orientation retained across the substeps of one world step:
...
static float
padded_half_width(const AabbWorld *world, const AabbBody *body)
{
	return (body->width + world->padding) / 2.0f;
}

static float
padded_half_height(const AabbWorld *world, const AabbBody *body)
{
...
aabb_world_add_body(AabbWorld *world, const AabbBodyDef *definition)
{
	...
	cpFloat mass = 1.0 / inverse_mass;
	cpBody *chipmunk_body = cpBodyNew(mass, INFINITY);
...
static bool
body_set_position(AabbWorld *world, AabbBodyId body,
		float center_x, float center_y)
{
...
aabb_world_max_penetration(const AabbWorld *world)
{
	...
	for (size_t a_id = 0; a_id < world->count; a_id++) {
		...
		for (size_t b_id = a_id + 1; b_id < world->count; b_id++) {
			...
			float required_x = padded_half_width(world, a)
				+ padded_half_width(world, b);
			float required_y = padded_half_height(world, a)
				+ padded_half_height(world, b);
...
enum {
	...
	CONTACT_AXIS_X_NEGATIVE = -1,
	CONTACT_AXIS_X_POSITIVE = 1,
	CONTACT_AXIS_Y_NEGATIVE = -2,
	CONTACT_AXIS_Y_POSITIVE = 2
};
		...
		float substep_dt)
{
	...
	/* Prefer the axis along which the current motion is separating the pair. */
	...
	if (solve_x) {
		...
		return direction > 0
			? CONTACT_AXIS_X_POSITIVE : CONTACT_AXIS_X_NEGATIVE;
	}
	...
	return direction > 0
		? CONTACT_AXIS_Y_POSITIVE : CONTACT_AXIS_Y_NEGATIVE;
}
		...
		float substep_dt)
{
	...
	if (axis == CONTACT_AXIS_X_POSITIVE
			|| axis == CONTACT_AXIS_X_NEGATIVE) {
		float penetration = required_x - sign * dx;
	...
	float penetration = required_y - sign * dy;
	float relative_normal_velocity =
...
static bool
solve_contacts(AabbWorld *world, float substep_dt)
{
	bool changed = false;
...
aabb_world_add_body(AabbWorld *world, const AabbBodyDef *definition)
{
	...
	body->width = definition->width;
	body->height = definition->height;
	body->inverse_mass = 1.0f / (definition->width * definition->height);
...
aabb_world_step(AabbWorld *world, float dt)
{
	...
	for (unsigned int step = 0; step < substeps; step++) {
		...
		for (unsigned int iteration = 0;
				...
				iteration++) {
			if (!solve_contacts(world, substep_dt))
				break;
		}
...
aabb_world_required_dilation(const AabbWorld *world, float clearance)
{
	...
	for (size_t a_id = 0; a_id < world->count; a_id++) {
		...
		for (size_t b_id = a_id + 1; b_id < world->count; b_id++) {
			...
			float required_x = padded_half_width(world, a)
				+ padded_half_width(world, b) + clearance;
			float required_y = padded_half_height(world, a)
				+ padded_half_height(world, b) + clearance;
		...
		float center_x, float center_y, float scale)
{
	...
	for (size_t i = 0; i < world->count; i++) {
		float x, y;
		if (!aabb_body_get_position(world, (AabbBodyId) i, &x, &y)
				|| !body_set_position(world, (AabbBodyId) i,
