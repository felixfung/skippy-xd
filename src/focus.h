...
focus_miniw_adv(session_t *ps, ClientWin *cw, bool move_ptr) {
	...
	cw->mainwin->client_to_focus->focused = 1;
	ps->active_mainwin = cw->mainwin;
	clientwin_render(cw);

	printfdf(false, "(): ");
	printfdf(false, "(): client_to_focus = %p", cw->mainwin->client_to_focus);
}

static inline void
focus_miniw(session_t *ps, ClientWin *cw) {
	focus_miniw_adv(ps, cw, ps->o.moveMouse);
}
